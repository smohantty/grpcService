...
# Test script for the new notification API
...
start_notification_client() {
    ...
    (
        echo "y" | timeout 30s ./build/image_client --name "$client_name" --test-notifications > "$log_file" 2>&1
    ) &
...
echo "   (Clients will run for 30 seconds)"
echo ""
